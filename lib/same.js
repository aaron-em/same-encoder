...
SAME.constructMessageByteArray = function(message) {
  ...
  if (message !== null) {
    // message header
	//ARRAY.push("Item1", "Item2, "etc"); appends to array
    msgContent = [
...
SAME.encode = function(message) {
  var returns = this.validateMessage(message);
  var validationErrors = returns[0];
  var messages = returns[1];
  console.log("HELLO. Message length: " + messages.length);
